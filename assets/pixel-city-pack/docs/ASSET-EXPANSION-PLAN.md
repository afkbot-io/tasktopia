...
1. [x] `npm run assets:build` — 193 семейства, 965 стадий, 208 props и 8 моделей транспорта.
...
**Транспорт и остановки:** восемь самостоятельных моделей машин 16×8 в трёх нарисованных направлениях: боковой вид на восток, отдельный задний вид на север и отдельный передний вид на юг. Запад — единственное допустимое зеркалирование. Один канонический парный контракт остановки 16×16 (`bus-stop-horizontal`, `bus-stop-vertical`) и городской автобус 48×16 (6×2 клетки) с теми же тремя видами (`city-bus-horizontal`, `city-bus-north`, `city-bus-south`). Автобус допускается только на трёхклеточных дорогах. Старые несовместимые семейства удалены, а не скрыты fallback-веткой.
...
- Принятый AI-authored набор из трёх стадий с проверенным SHA-256: 193 семейства.
- Очередь AI-перерисовки: 0 семейств.
- Последний принятый пакет: 50 семейств — 14 жилых, 31 коммерческое и 5 civic/landmark; полный перечень закреплён контрактом `authoredBatch50` в `tests/pixel-city-assets.test.ts`.

### 16.2. Единая миграция всех зданий на V5-сетку

- Каталог миграции охватывает все 193 семейства, а не только здания с тегом
  `new-build`.
- Принято в формате трёх независимых late-stage sources: 193.
- Очередь последовательной художественной миграции закрыта.
