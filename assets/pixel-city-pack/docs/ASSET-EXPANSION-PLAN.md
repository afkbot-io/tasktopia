...
видов животных. Инциденты используют три структурно разные тяжёлые пожарные машины
`56×24`/`54×22` с footprint `7×3` (автоцистерна, спасательная и автолестница)
из immutable V7-листа в той же near-top проекции, масштабе и плотности деталей,
что городской автобус. East-facing authored-спрайт зеркалится только при парковке
справа от фасада, чтобы кабина и струя были направлены к очагам:
наклонное лобовое стекло, высокая кабина, видимые верхняя и ближняя боковая
плоскости и крупные округлые колёса; и четыре кадра огня
...
1. [x] `npm run assets:build` — 193 семейства, 965 стадий, 288 props и 8 моделей транспорта.
...
- Всего в активном каталоге: 167 семейств зданий.
- У всех 167 семейств есть отдельные authored-файлы стадий 3–5 и SHA-256;
  это инвентарный минимум, а не автоматическое подтверждение V5-качества.
- Строгий V5 gate пройден у 167 семейств: geometry contract, независимые
  стадии, door-scale, baseline и native-grid preview.
- Строгая очередь закрыта; удалённые 36 частных домов не считаются активными
  семействами и не возвращаются в runtime.
...
- Каталог миграции охватывает все 167 активных семейств, а не только здания с тегом
  `new-build`.
- Независимые late-stage sources и строгая V5-приёмка присутствуют у всех 167
  активных семейств.
- Для десяти обычных малоэтажных корпусов дополнительно обязательны ширина
  `18–24` клетки, глубина `14–16`, двойная дверь `16×16`/`12×14` и
  `doorVisualReview`, проверяемый отдельным контактным листом.
