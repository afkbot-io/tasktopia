# План расширения Tasktopia Pixel City Pack — архив V4/V5

**Статус: SUPERSEDED, 2026-09-05.** Исторический план версии 2.0 сохранён
ниже для контекста, а не как инструкция к текущему runtime. Его количества,
размеры, инструменты и отмеченные «выполнено» относятся к прежнему каталогу;
они не означают, что старые ресурсы доступны после compact/dense cutover.

## Действующее направление вместо старого плана

Авторитетные документы: [GENERATION-SPEC](GENERATION-SPEC.md),
[compact cutover](../../../docs/COMPACT-BLOCK-CUTOVER.md),
[геометрия зданий](../../../docs/art/COMPACT-BUILDING-ART-CONTRACT.md)
и [micro ambient](../../../docs/art/MICRO-AMBIENT-ART-CONTRACT.md).

- Зарегистрированы три независимые семьи: apartment `48×48 px / 6×6` клеток,
  row `48×24 / 6×3`, wide `48×32 / 6×4`; базовая клетка — `8×8 px`.
  Одна геометрия сохраняется внутри семейства на стадиях `5 → 4 → 3`;
  размеры разных семейств не получают растягиванием одного изображения.
- Новые плотные прямоугольные кварталы используют сохранённые планы version 3;
  существующая геометрия version 2 сохраняется. План содержит совместимые
  слоты, проходы и task-owned карманные парки. Дороги хранятся семантическими
  сегментами общих границ, а не прежними поклеточными строками.
- Школа, медицина, службы, магазин, железная дорога и аэропорт пока являются
  сервисными резервами следующего подходящего слота по порогам compiler.
  Задача создаётся только авторизованным запросом; резерв не равен готовому
  зданию. Особая графика служб и полноценные транспортные комплексы —
  последующие подэтапы, не восстановление большого старого каталога.
- Новый micro-набор содержит 36 кадров людей, машин, статичных животных
  и самолёта. Произвольная мелкая россыпь декора удалена; фактура земли
  остаётся в terrain raster, леса и осмысленный декор парков сохраняются.
- CITY scene v3, COUNTRY overview v5 и PLANET atlas v4 используют одну
  географию: на стране один квартал представлен одним зданием, на планете —
  один район. Полёты требуют завершённых task-backed аэропортов.

Перед расширением сначала согласовываются новая геометрия и её тесты, затем
отдельные AI-источники `5 → 4 → 3`, визуальная приёмка, публикация и проверка
в настоящей сцене. Проверки: `assets:compact:verify`, `assets:build`,
`assets:verify` последовательно. Production-релиз этим документом не заявлен.

## Исторический текст версии 2.0 — не нормативный

**Историческая цель:** довести разнообразие городов до уровня классических
city-builder-ов, развивать уникальный Государственный архив и объекты
1-на-город, увеличить число вариантов зданий, заправок, фонарей и парков.
