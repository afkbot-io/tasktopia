...
Четыре новых исходника созданы встроенным ImageGen для текущей задачи. Краткая спецификация генерации (не дословная запись запросов):

- `cars-extra-v2.png`: сетка 4×4; пикап, микроавтобус, хетчбэк, доставка. Фактические направления строк: север, запад, юг, восток — это учтено при нарезке.
- `animals-directions-v2.png`: сетка 8×4; лиса, олень, кролик, кабан, утка, овца, собака, кошка; строки север, восток, юг, запад.
- `boats-v2.png`: сетка 2×2; гребная лодка, рыбацкий катер, буксир, парусная лодка. Строгий верхний вид, северный нос, приглушённая палитра.
- `clouds-v3.png`: четыре облака из ступенчатых квадратных цветовых групп в палитре ландшафта, без мягких градиентов.

Общий замысел: читаемые маленькие силуэты, жёсткие края, прозрачный/удаляемый фон, без надписей. Исходники и итоговая контактная таблица просмотрены основным агентом. Publisher `build-visual-refresh.py` не перерисовывает здания и ограничивается лодками, облаками и регистрацией микроспрайтов.
