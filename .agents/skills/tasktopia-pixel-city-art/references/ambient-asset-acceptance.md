...
- Parked fire engines use a `48x16 px` east-facing canvas with an exact `46x14 px` connected opaque body and a `6x2` visual footprint. This is two current passenger-car canvases long and remains below the `56x24 px` bus envelope. The accepted side view must share the passenger-car camera: stepped cab roof, readable side window plus sloped front windscreen, tapered hood, shallow top plane, darker near-side plane and two rounded wheels on the same baseline. The silhouette may fill at most 75.5% of its occupied rectangle, needs at least six distinct opaque row widths and five front-edge steps, may contain at most three full-width rows, and ends in two separate 3 px wheel contacts. Pumper, rescue and ladder variants require different tank/bay/ladder silhouettes; a rectangular elevation, palette-only variants, disconnected wheels/equipment or a stale source are blocking.
