...
- Moving residents use `16x24 px` canvases with a compact `8–12x16–18 px` opaque subject: separately authored north/rear, east and south/front views, each with three readable walk poses. West may mirror the accepted east cycle. Their feet share one bottom-centre anchor on the destination path cell; the playback loop is `A→B→C→B`, never a direct opposite-contact jump. A centred `8x8` icon or a single bobbing frame is obsolete.
