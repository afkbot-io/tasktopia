...
- Emergency road vehicles are measured against the normal car family. A fire engine is `48×16 px` with a `46×14 px` opaque body — exactly two `24×16 px` passenger-car canvases long, while remaining shorter and lower than the `56×24 px` city bus. It keeps the same horizontal frontal-top roof visibility and road baseline as cars; ladders and roof equipment stay within the authored vehicle silhouette. Pumper, rescue and ladder variants must differ structurally rather than by palette alone.
