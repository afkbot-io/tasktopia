...
- Emergency road vehicles are measured against the normal car family. A fire engine is `48×16 px` with a `46×14 px` opaque body — exactly two `24×16 px` passenger-car canvases long, while remaining shorter and lower than the `56×24 px` city bus. It keeps the same near-top frontal-side camera and road baseline as cars: stepped cab roof, separate side glass and sloped windscreen, tapered hood, shallow equipment top plane, darker near-side body and two rounded wheels. A rectangular side elevation or roof-only plan is blocking. Pumper, rescue and ladder variants must differ structurally rather than by palette alone; ladders and roof equipment stay inside the authored silhouette.
