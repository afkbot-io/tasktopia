...
- A walking resident canvas is `16×24 px`; the opaque figure uses one 18-pixel-high family scale, hard alpha, one anchored baseline and three visibly distinct authored poses per direction. The passing pose may narrow naturally and must never be independently enlarged to fill the envelope. In the A/C overlay, changed lower-body pixels must appear on both sides of the body axis, proving that opposite feet lead; a palette-only or same-leg change is blocking. West is a pixel-exact mirror of east; north and south remain separately authored. Runtime returns through the passing pose (`A→B→C→B`) so opposite contacts never snap directly into each other. Dialogue/UI is a separate foreground layer and is never baked into the sprite.
...
- Emergency road vehicles are measured against both the normal car family and the city bus. A heavy fire engine uses the bus-class `56×24 px` canvas, a connected `54×22 px` body and a `7×3` footprint. It keeps the same near-top orthographic camera, detail scale and road baseline: tall crew cab, separate side glass and sloped windscreen, shallow cab/equipment top planes, darker near side and two large rounded wheels. A small pickup-like body, pure side elevation, rectangular primitive or roof-only plan is blocking. Pumper, rescue and ladder variants must differ structurally rather than by palette alone; ladders and roof equipment stay connected inside the authored silhouette.
