import { atlasGridPath, atlasRailRoutes } from "./atlas-grid-transport";
...
export function countryRailways(overview: CountryOverviewDto) {
  ...
  const stops=overview.cities.flatMap(city=>[...(city.miniature.stations??[])].sort((a,b)=>a.taskId.localeCompare(b.taskId)).slice(0,1).map(station=>{
  ...
  const routes = overview.railConnections === undefined ? atlasRailRoutes(stops,land) : overview.railConnections.flatMap(route=>{
    ...
    return cells.length ? [{id:route.id,from,to,cells}] : [];
  });
  const domestic = routes.map(route=>({id:route.id,fromCityId:route.from.cityId,toCityId:route.to.cityId,fromStationId:route.from.id,toStationId:route.to.id,points:[positions.get(route.from.id)!,...route.cells.map(cell=>({x:(cell.x+.5)*cellSize,y:(cell.y+.5)*cellSize})),positions.get(route.to.id)!],progressRange:undefined as [number,number]|undefined}));
  const foreign=(overview.railConnections??[]).filter(route=>route.points && route.points.length>1)
    .map(route=>({...route,points:route.points!,progressRange:route.progressRange}));
