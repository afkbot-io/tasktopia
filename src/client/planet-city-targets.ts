...
export function layoutPlanetCityLabels(cities: readonly PlanetCityTarget[], width: number, height: number) {
  ...
  for (const city of [...cities].sort((a,b)=>a.id.localeCompare(b.id))) {
    if (city.center.x<0 || city.center.x>width || city.center.y<0 || city.center.y>height) continue;
    const w=Math.min(180,Math.max(90,city.name.length*7+24)), h=26;
    const x=Math.max(4,Math.min(width-w-4,city.center.x-w/2));
    const start=Math.max(4,city.center.y-Math.min(city.radius,70)-h-8);
    ...
    const y=slots.find(y=>y>=4 && y+h<height-4 && placed.every(p=>x+w+4<=p.x || x>=p.x+p.width+4 || y+h+4<=p.y || y>=p.y+p.height+4));
