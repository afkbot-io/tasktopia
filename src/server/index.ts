...
io.use((socket, next) => {
  const user = getSessionUser(db, cookieValue(socket.handshake.headers.cookie, SESSION_COOKIE));
...
app.get("/mcp", async (_request, reply) => reply.code(405).send({ error: "METHOD_NOT_ALLOWED", message: "MVP MCP работает stateless через POST" }));
