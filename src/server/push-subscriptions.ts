...
export function normalizePushSubscription(input: unknown): BrowserPushSubscription {
  ...
  const endpoint = approvedPushEndpoint(candidate.endpoint);
...
export async function savePushSubscription(db: Db, userId: string, input: unknown): Promise<BrowserPushSubscription> {
  ...
  const saved = await db.prepare(`INSERT INTO push_subscriptions_v1
    ...
    ON CONFLICT (endpoint_hash) DO UPDATE SET p256dh=EXCLUDED.p256dh, auth=EXCLUDED.auth,
      ...
      WHERE push_subscriptions_v1.user_id=EXCLUDED.user_id
    RETURNING user_id`)
    .get<{ user_id: string }>(randomUUID(), userId, subscription.endpoint, endpointHash, subscription.keys.p256dh,
      subscription.keys.auth, subscription.expirationTime, timestamp, timestamp);
  if (!saved) throw new Error("PUSH_ENDPOINT_OWNED");
  return subscription;
}
