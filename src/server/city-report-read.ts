...
export async function readCityReport(
  ...
  const result = await db
    ...
    SELECT t.id, t.task_number, t.title, t.status, t.progress, t.due_at, c.name AS city_name, d.name AS district_name, u.name AS assignee,
    ...
    SELECT DISTINCT e.payload_json->>'taskId' AS task_id FROM events e
    WHERE e.country_id=? AND e.type='task.status_changed' AND e.payload_json->>'status'='COMPLETED'
      AND e.created_at>=CURRENT_TIMESTAMP - (?::int * INTERVAL '1 day')
  ), classified AS (
    SELECT *, (defects>0 OR (status<>'COMPLETED' AND (due_at<CURRENT_TIMESTAMP OR (assignee IS NULL AND status<>'PLANNING') OR dependencies>0 OR status='TESTING'))) AS attention,
    ...
    jsonb_build_object('working',COUNT(*) FILTER(WHERE status IN ('STARTED','IN_PROGRESS','TESTING')),
      ...
      'completedPeriod',COUNT(*) FILTER(WHERE status='COMPLETED' AND classified.id IN (SELECT task_id FROM completions))) AS counts,
