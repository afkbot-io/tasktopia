...
Обновление карты и интерфейса: [реализация, проверки и замеры](docs/PLANET-CITY-UI-2026-09-22.md).
...
- каталог содержит 57 компактных семейств зданий (28 жилых, девять служб и 20 уникальных городских объектов), включая длинные горизонтальные и вертикальные корпуса, Г- и П-образные дома; парки, вода и парковка занимают собственные task-слоты. Большой прежний каталог удалён;
- уникальные типы выдаются по одному на город и распределяются по кварталам; все [20 типов](docs/art/CITY-LANDMARKS-2026-09-09.md) подключены с пятью стадиями строительства;
- здания проходят пять стадий, а ещё не занятые слоты показываются отдельной стадией 0 планировки;
- маленькие люди и автомобили используют отдельные top-down микроизображения; направления выбираются из подготовленных видов, животные также имеют четыре направления;
...
- Карта содержит Планету, Город и слой Районов. PLANET v4 показывает города миниатюрами из 24 модулей; CITY v4 получает общую сцену, снимки завершённых районов и ограниченный список соединений реальных аэропортов. Межгородские наземные дороги используют один сохранённый план; отсутствие сухопутного пути показывается явно, а не скрывается фиктивным мостом;
...
|---|---:|
| Семейства зданий | 57 |
| Строительные стадии зданий | 285 |
| Props и городской декор | 120 |
| Модели транспорта | 8 |
| Микроизображения людей, машин, животных и самолёта | 76 |
| Terrain families | 12 |
| Все runtime PNG | 682 |
| Зарегистрированные MCP tools | 47 |

Число PNG включает весь `public/game-assets/v5`, в том числе атласы.
Восемь моделей машин входят в новый `microAmbient`,
...
- **SVG/Canvas/PixiJS** разделяют уровни карты: PLANET использует SVG, CITY — PixiJS, Районы — слой той же городской сцены; серверные чанки остаются внутренней проекцией хранения, но не единицей сетевой навигации внутри города.
