...
В проверяемой ветке каталог и manifest содержат 57 семейств. Число сверено с обоими
JSON-источниками; прежнее описание 167 семейств не относится к текущему compact runtime.
Пять профилей новых кварталов — садовый малоэтажный, современный, смешанный городской,
торговые улицы и общественный центр — задаются building-profiles.ts. Выбор воспроизводим;
дома могут повторяться, каталог 20 городских landmarks имеет собственные ограничения
уникальности. Панель развития различает неоткрытый, запланированный, строящийся, готовый
и исторический объект; не объявляет службу готовой только по числу кварталов.

Порт compact-port-v1 имеет native48×32, якорь 24,32, роль PORT и maxPerCity1. Пять стадий
терминала сопровождаются стадиями пирса. Его размещение требует отдельного валидированного
плана берега и воды; один лишь выбор семейства не обходит эту проверку. Исторические
участки и транспортные коридоры остаются зарезервированными.
