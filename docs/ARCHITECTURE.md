...
Overview содержит 256 terrain samples, ограниченный набор общих PATH-связей и не содержит decorations, world features, nearby-city expansion или описаний задач; detail содержит 4096 клеток и номер с названием задачи, а полное описание читается из task detail. JSON-ответы от 1 KiB сжимаются Brotli/gzip. Клиент держит только viewport + 0,25 viewport prefetch: до восьми JSON-чанков читаются независимо от очереди декодирования PNG, а до шести наборов критичных спрайтов готовятся параллельно. Машины, жители, животные и самолёты загружаются отдельным необязательным ambient-набором и не блокируют землю, дороги и здания. Параллельные чтения одного chunk/LOD объединяются; законченные ответы хранятся в LRU до 160 записей, готовый detail локально даёт overview без сети, а готовый ground хранится до 96 чанков. Обычное панорамирование в пределах одного LOD устанавливает готовые чанки постепенно и не отменяет уже начатое полезное чтение, но адресная realtime-инвалидация отменяет устаревший запрос изменённого чанка. Detail/overview переключается с гистерезисом: данные и ассеты нового масштаба готовятся параллельно поверх старого цельного кадра, затем все видимые ground-чанки меняются одним синхронным кадром. Сервер держит LRU до 512 готовых чанков и объединяет одинаковые холодные чтения; структурные события очищают cache страны, изменения статуса, рабочих полей, дефектов и метаданных города/района — только пересекающиеся ключи, а паспорт страны и комментарии геометрию не инвалидируют.
