...
MCP `task.create` принимает `visualKind: "PARK"` и `parkVariant` из единого
`TASK_PARK_VARIANTS`. Все 15 ключей, размеры и стадийность описаны в
[контракте общественных пространств](art/TASK-PUBLIC-SPACES.md).
`parkVariant` без `visualKind` означает PARK; сочетание с BUILDING отклоняется.
`urban-large` требует участок `17×17`, а не растянутый рисунок малого парка.
Выбранный вид сохраняется при последующих синхронизациях и переносе задачи.

При нехватке совместимых слотов добавляются кварталы того же спринта на
связанном дорожном фронтире города. Другой спринт или задача при этом не
создаются. MOVE и руины остаются занятыми. Новые variant/template keys требуют
совместного обновления клиента, сервера и миграции `0027`; сброс мира не нужен.
Положение PLANET-маркера района берётся из принадлежащего ему занятого
квартала, а не из пустого центра общей рамки разнесённых кварталов.
...
`comment` необязателен, до 4000 символов; остальные поля обязательны. Схема строгая: неизвестные поля отклоняются. ID, номер, статус, история и служебная роль задачи сохраняются; прежняя площадка навсегда занимает свой участок маркером MOVE, ведущим прямо к текущей задаче. Новое размещение и старый маркер записываются атомарно; ошибка доступной площади не оставляет частичный перенос. Повтор неизменного запроса с прежним ключом не создаёт дополнительный маркер. Перенос в другую страну/город или завершённый/заброшенный спринт запрещён.
