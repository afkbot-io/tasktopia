...
## Изоляция полного браузерного набора

После включения loopback-сценариев полный локальный набор дал100passed/14failed/
54skipped. Причины14падений: десять зависели от оставленного старым app-тестом
имени «Тестовая страна 2», два использовали прежнее ожидание лишней transport
инвалидации, два возвращали камеру полным обратным drag после достижения её
ограничения и проскакивали исходный город. Это ошибки согласования тестов,
не основание выключать новые сценарии.

App-тест теперь возвращает исходное имя страны в finally через проверяемый API.
Delayed-connect тест требует ровно одну сцену и ноль лишних transport refresh,
сохраняя принудительный порядок подключения до первого кадра. Label-тест ждёт
ухода перехода и возвращается от границы небольшими жестами до появления исходных
подписей; проверка отсутствия новых запросов сохраняется.

Повтор всех затронутых сценариев вместе с app-тестом и последующими потребителями:
27/27 Chromium/WebKit прошли за1,5мин, `/tmp/living-world-ci-isolation-browser.log`.
Typecheck, scoped ESLint, diff-check прошли. Повторно просмотрен весь diff трёх
тестов: проверки результата не удалены, production code не менялся.
CI35129721473 остановлен как устаревший до публикации этих исправлений; он не
объявляется успешным. Следующий обязательный CI проверит окончательный commit.
