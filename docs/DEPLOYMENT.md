...
до замены тега `latest` закрепляет recovery image отдельным тегом и private
`docker image save` архивом, проверяет его checksum и настоящий `image load`
с совпадением image ID. Кандидат получает такую же защиту. Если image-store
ссылка прежних контейнеров уже потеряна, восстанавливается сборка из точного
`--previous-revision` в отдельном git worktree внутри private run directory.
Исходный и заново собранный image ID записываются раздельно: новый образ не
выдаётся за побайтную копию потерянного. Прежний Compose использует этот
закреплённый recovery image; неизменённые исходные контейнеры также остаются
допустимы для раннего отката. Не допускается смесь образов между тремя ролями.
Ошибки сохранения/загрузки/сборки прерывают подготовку до maintenance.
...
Если при раннем recovery все три исходных контейнера сохранились с прежним
image ID и проверенными environment/mount/network, запускаются именно они,
без ненужного recreate. После запуска остаётся обязательная проверка health.
...
Fingerprint схемы нормализует только ассоциативную группировку `AND`
(и случайные ключи psql `\restrict`): PostgreSQL разворачивает `BETWEEN` в составной `CHECK`, а
при восстановлении может убрать внутреннюю пару скобок. Границы, операторы,
`OR`/`NOT`, casts, литералы и тела функций остаются частью сравнения. Данные,
последовательности и metadata сравниваются полностью. После изменения алгоритма
fingerprint нужен новый prepare run с новой копией; старый journal не переиспользуется.
