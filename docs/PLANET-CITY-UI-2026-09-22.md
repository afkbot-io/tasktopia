...
| AC005 — загрузка | Один HTTP-снимок города, нет скрытого Pixi до намерения входа; ограниченные предварительная загрузка и DTO-кэш. Первый кадр ждёт необходимую графику. Повтор и отмена при ошибке; возврат переиспользует canvas. | `city-asset-overlap`, `map-retry`, `map-streaming`, `navigation-cache`, замеры ниже |
...
| --- | --- | --- | --- |
| Desktop, вход p95 (10 проб) | 1 231 мс | 1 085 мс | <2 000 мс |
| Desktop, медиана входа | 989 мс | 1042 мс | — |
| Desktop, возврат p95 (10 проб) | 171 мс | 44 мс | <500 мс |
| Mobile 4× CPU, максимум из 3 входов | не измерялось | 1 580 мс | <4 000 мс |
| CITY scene GET / chunk+viewport GET за вход и возврат | 1 / 0 | 1 / 0 | 1 / 0 |
| Главный поток за 5 секунд, среднее двух окон | Полная: 5 037 мс | Экономная: 2 062 мс (−59,1%) | Снижение нагрузки |

При небольшом числе проб p95 вычислен методом ближайшего ранга. Первый вход не стал быстрее во всех пробах: типичный вход немного дольше базы, но укладывается в бюджет; возврат ускорился. В каждом финальном desktop-прогоне сохранён тот же canvas, HTTP-ошибок нет. В сравнении детализации машин стало 8 вместо 17, прохожих — 12 вместо 29, строителей — 2 вместо 8; DPR 2→1, предел FPS 60→30, повторных ground rebuilds не прибавилось.
