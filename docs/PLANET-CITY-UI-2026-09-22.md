...
| AC008 — городская среда | Составные общественные пространства: места пикника, велостойки, площадки, прогулочные скамейки, сады и смешанные посадки. Смягчена плотность травяной текстуры; небольшие тени и декор размещаются с учётом занятости. Небольшой указатель района занимает только свободные 2×2 клетки. Сводка готовых, активных, проверяемых и планируемых задач открывается в боковом списке; плавающая плашка над дорогой удалена. | `task-public-spaces`, `green-area-paths`, `compact-area-stages`, `district-development`, существующие пять профилей зданий |
...
После исправления подготовки все сценарии первого кадра и пять профилей застройки проходят на Linux. Оставшийся парковочный сценарий также переведён на часы симуляции для заезда: за 45 секунд ожидания в трассе прошло только 10,4 секунды симуляции, машина уже въезжала на участок. Повтор точного снимка CI в контроллере дал остановку на 11,85 секунде и возврат на дорогу на 26,4 секунде, без столкновений. Проверка сохраняет границы 45 секунд симуляции для заезда и 25 для остановки с выездом, прослеживая ту же машину. Непрерывные миниатюры кадров в trace отключены, как в существующем нагрузочном тесте движения; DOM, исходники и явные скриншоты сохранены. Участок проверяется в окне 800×600. Общий таймаут этого теста и бюджеты первого кадра не менялись.

Удалённые COUNTRY-only браузерные тесты заменены проверками планетных подписей, пиксельного масштаба, прямого входа, движения транспорта и ошибок. Серверные проверки `/overview` и геометрии сохранены для совместимости. Опциональные тесты крупных городов переведены на новую навигацию, но их длительные прогоны в локальную приёмку не входят. Полный обязательный набор запускает CI PR, включая все unit-тесты, генерацию/проверку ассетов, recovery, MCP smoke, браузеры и audit; его результат привязан к SHA в PR. Измеренная полная матрица заняла более 44 минут, поэтому общий предел задания увеличен до 60 минут; таймауты отдельных сценариев и измерений сохранены.
...
1. Открыть приложение: видна планета без нижней панели; Планета / Город / Районы доступны после входа в город. Приблизить реальный город, открыть подписью, затем вернуться. Повторить Enter и жестом на телефоне; камера не должна сбрасываться.
...
## Дополнение: проверка UI и быстрый вход в задачу

После исходной реализации добавлен отдельный вход по ссылке: сначала карточка, затем город по закрытию. Доработаны PWA/превью, ошибки и авторизация, контраст, мобильные размеры и доступность вкладок. [Подробности, отдельный замер до/после и снимки](TASK-LINK-ENTRY-2026-09-22.md). Замеры навигации выше относятся к ранее зафиксированной версии рендера; новые замеры входа в карточку имеют собственную фиксацию исходников и не подменяют их.

Дополнение 23 сентября: [указатель района без перекрытий и безопасная область PWA](evidence/task43/district-marker/README.md).
