...
1. Прогнать `verify_building_stages.py` из навыка `tasktopia-building-stage-verifier` отдельно для `civic-hospital-v5`, `commercial-gas-station-v5`, `commercial-parking-lot-v5` и `state-archive-core-v5`: `acceptedByCode` должен быть `true`, а `errors` и `warnings` — пустыми.
