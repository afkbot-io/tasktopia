...
2. При 20 wheel-событиях zoom непрерывно проходит промежуточные значения и не превышает `2.6`, города не стягиваются друг к другу, а Long Tasks API не фиксирует задачу дольше 50 ms. Raster использует существующие terrain/pavement assets при `16×16 px` на geography-клетку; полноразмерные PNG зданий в Network страны не запрашиваются. `data-country-selected-cells > 0`; соседняя суша и вода совпадают с PLANET fixture; число `.country-atlas-aircraft` равно маршрутам, и transform хотя бы одного самолёта меняется во времени.
