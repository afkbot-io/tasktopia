...
1. Открыть страну: существует один `.country-overview` canvas, SVG отсутствуют, DOM всей страницы не превышает 1 500 узлов. Ответ `/api/countries/:countryId/overview` имеет `schemaVersion: 3`, содержит bounded terrain `36×22`, города, агрегаты районов и semantic miniatures, но не содержит полную геометрию города, здания, дороги, surfaces, props или sprite URL; payload не превышает 200 KB.
2. При 20 wheel-событиях zoom не превышает `2.6`, города не стягиваются друг к другу, а Long Tasks API не фиксирует задачу дольше 50 ms. Полноразмерные PNG зданий в Network страны не запрашиваются.
