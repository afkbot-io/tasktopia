...
## Указатель района и безопасная область

- В плотной застройке постоянные таблички не закрывают улицы и здания. Малый указатель 16×16 px занимает только свободные 2×2 клетки внутри района, включая полную графику и область нажатия. Если места нет, знак не размещается; список районов доступен через название города в шапке.
- Нажатие на указатель открывает боковую сводку и задачи района. Проверить закрытие, повторное открытие, 320/390 px и 1440×900/1100. После движения карты и изменения окна координаты знака сохраняются, новых scene/chunk/viewport-запросов нет.
- На телефоне с вырезом экрана открыть прямую ссылку в PWA в обеих ориентациях: крестик, вкладки и низ карточки остаются вне системных зон. CDP-эмуляция проверяет CSS; физическая PWA остаётся отдельной проверкой устройства.
