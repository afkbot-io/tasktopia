...
1. Открыть страну: существует один `.country-overview` canvas, SVG отсутствуют, DOM всей страницы не превышает 1 500 узлов. Ответ `/api/country-overview` имеет `schemaVersion: 1`, содержит seed, города, агрегаты районов и связи, но не содержит клетки, здания или sprite URL; payload не превышает 200 KB.
...
15. Изменение статуса задачи обновляет агрегат города; комментарий не вызывает новый overview fetch. Структурное событие получает одну новую revision `/api/country-overview`.
