...
3. Открыть большой город: до готовности всей сцены виден loader; выполняется один `/api/cities/:id/scene` и ни одного `/api/world/viewport` или `/api/chunks/*`. После появления карты drag и wheel не создают запросов данных.
