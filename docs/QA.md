...
7. После десяти циклов `Планета → Страна → Город → Страна` одновременно остаётся один canvas уровня; число scene asset leases возвращается к текущей сцене, а размонтированные Pixi renderers не остаются в DOM.
...
16. Запустить `npm run test:atlas`: сценарий проверяет десять городов, WebGL canvas без SVG, безопасный hover/click, first frame `<2 s`, raw payload `<200 KB`, bounded zoom, один city-scene request, нулевой pan I/O, rollback при ошибке preload и десять разных city eviction cycles.
