...
1. Открыть атлас страны: terrain закрывает экран квадратными блоками без зубчатого нижнего края и одноцветных полей; полноразмерные PNG зданий в Network не запрашиваются. Ответ `/api/country-atlas` имеет `schemaVersion: 5`, содержит `terrainSeed`, `labelAnchor` и `districts[].progress`, но не содержит `macroTerrain`/`cutoutTerrain`.
...
7. Все city cutout и подписи остаются внутри bounds с горизонтальным запасом не меньше 16 клеток; подпись касается северной части своего города, а не собирается в общей строке сверху. При viewport `1440×900` seeded terrain занимает всю высоту и ширину без растягивания клеток.
8. В шапке нет кнопки `Карта`, счётчиков районов/зданий, отдельной кнопки `План` и нижних help-плашек; `План страны` доступен из меню названия страны. Кнопка аккаунта квадратная, высота шапки на desktop равна `52 px`.
...
10. На стране сравнить подписи самого короткого и самого длинного названия: рамки должны иметь один размер. Один город не должен автоматически растягиваться крупнее масштаба карты с несколькими городами.
...
12. На планете вращать шар drag и стрелками, приблизить wheel: рамки стран остаются `144×38`, не пересекаются и нажимаются. Облака проходят через центральную область шара; самолёт меньше городского варианта и уменьшается у начала/конца маршрута.
10. На планете отображаются только страны текущего аккаунта. Их hex-области не пересекаются, крупная страна визуально больше маленькой, progress читается в подписи; клик и Enter переключают заголовок и атлас на ту же страну.
...
9. Изменение статуса задачи обновляет только её миниатюру и progress района; комментарий не вызывает новый atlas fetch. После reload с задержкой `/api/country-atlas` на `1500 ms` snapshot из `sessionStorage` должен появиться быстрее `700 ms`.
10. Запустить `npm run test:atlas`: сценарий проверяет десять городов, четыре профиля домов, отсутствие полноразмерных PNG, безопасный hover/click, компактную шапку, session-cache и бюджет hot-cache `≤ 1500 ms`, raw payload `< 2 MB`.
