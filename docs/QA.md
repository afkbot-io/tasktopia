...
- Сравнить CITY/COUNTRY/PLANET: один набор реальных ориентиров, чёткие пропорции при
...
1. Открыть страну: существует один `.country-overview-raster` canvas без scene SVG/WebGL, DOM всей страницы не превышает 1 500 узлов. Ответ `/api/countries/:countryId/overview` имеет `schemaVersion: 7`, содержит geography terrain/territory, города, агрегаты районов и `miniature.blocks` / `miniature.airports`. На город выбирается не более12реальных представителей кварталов и3транспортных ориентиров; полная геометрия CITY, дорожные cells и props не передаются. Старые `coverageCodes`, `shapeCodes`, `districtCodes` не являются контрактом новой миниатюры.
...
14. PLANET v4 и COUNTRY v7 показывают одинаковый ограниченный набор реальных ориентиров. CITY v4 получает согласованные AIR/RAIL/SEA-связи готовых задач; один готовый endpoint не создаёт рейс. Проверить исключение незавершённой и недоступной остановки, остановку состава у станции, общий серверный момент отправления и обновление открытой карты при удалении/завершении endpoint в другой стране. Удалённые направления не загружают чужие CITY-чанки. Пять стадий порта, аэропорта и вокзала сохраняют размер участка и пиксельный ракурс.
