...
- 10 городов с `BALANCED`, `DENSE_CORE`, `GARDEN_CITY` и `POLYCENTRIC`
  морфологией;
- 23 района и 92 задачи в основной стране;
- AUTO-задачи заполняют упакованные прямоугольные слоты семейств
  `compact-apartment-v1` (6×6), `compact-row-v1` (6×3),
  `compact-wide-v1` (6×4), включая остаточные парки;
- здания стоят на каменной площадке, парковые задачи — на парковой;
  стадии 1–2 сохраняют общий участок и одноклеточный отступ;
- дороги квартальной сетки имеют ширину 3 клетки;
- два реальных аэропорта в первом и последнем городах завершаются через
  обычные task/status API; COUNTRY V5 показывает ровно один маршрут между ними,
  а E2E сверяет аэропорты с taskId, ролью AIRPORT и стадией 5;
- для PLANET V4 дополнительно создаются две доступные именованные страны с
  одним городом, районом и тремя завершёнными задачами в каждой. Основная страна
  остаётся активной; другие страны fixture не удаляет;
...
Текущий совместный controller проверяется отдельным
`tests/e2e/city-mobility.spec.ts` и независимым
`tests/city-mobility-safety.test.ts`; спецификация и свежая evidence находятся в
[CITY-MOBILITY-DELIVERY.md](../CITY-MOBILITY-DELIVERY.md).
Старые CAR/BUS, gait и размеры 24×16 ниже — исторические проверки прежнего
runtime, не требования и не доказательства нового compact движения.

Актуальная проверка `tests/e2e/country-overview.spec.ts` требует V5 DTO:
`miniature.cellSize=8`, семантические `blocks/airports`, без старых
`districtCodes/terrainCodes` в миниатюре и без raw city geometry. Смена уровней
оставляет один скрытый CITY canvas с `map-active=false` и остановленной
анимацией; возврат в тот же город переиспользует canvas, другой город заменяет
его. Скриншот PLANET снимается только после `data-planet-ready=true`.
Десять COUNTRY-карточек проверяются попарно на отсутствие пересечений; их
screen-space упаковка и неинтерактивные линии не меняют координаты городов.
PLANET-карточки показываются только для территории, пересекающей одновременно
viewport и эллипс поверхности: `data-planet-countries` — полный доступный список,
`data-visible-countries` — реально видимые территории.

Запускайте seed только в выделенной локальной schema. Текущий профиль:
`compact_atlas_preview_20260905`, порт `5191`, Node 24; доказательства сохраняются
в `screenshots/dense-atlas-final`. Результаты старого world-validation ниже —
исторические и не заменяют текущий compact cutover gate.
...
- первый вход в любой город использует одинаковое detail-окно `160×100`, а кварталы имеют больше
